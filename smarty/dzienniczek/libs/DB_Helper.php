...
class DB_Helper {
	...
	function addHomework($classId, $subjectId, $homeworkDescr, $startDate, $endDate)
	{
		$con= pg_connect("host=$this->dbhost dbname=$this->dbname user=$this->dbuser password=$this->dbpass");
		$query='INSERT INTO homeworks (class_id, subject_id, description, date_start, date_end) VALUES ('.$classId.','.$subjectId.',\''.$homeworkDescr.'\',\''.$startDate.'\', \''.$endDate.'\');';
		$rs=pg_query($con, $query);
	}
